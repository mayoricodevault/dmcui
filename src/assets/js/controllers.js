...
App.controller('CompMapsGooglePNCtrl', ['$scope', '$localStorage', '$window','Pubnub',
    function ($scope, $localStorage, $window, Pubnub) {

        $scope.position={};
        $scope.allUsersMap = null;
        // Gmaps.js, for more examples you can check out https://hpneo.github.io/gmaps/
        ...
        Pubnub.getMessageEventNameFor('Channel-hkwogxhjp'), function (ngEvent, message, envelope, channelOrGroup, time, channel) {
            console.log(
                "Message Received." + "\n" +
                    ...
                    GMaps.geocode({
                        ...
                        callback: function (results, status) {
                            if ((status === 'OK') && results) {
                                ...
                                initMapOverlay(position.coords.latitude ,position.coords.longitude , completeStreet);
                                initMapSearch(position.coords.latitude ,position.coords.longitude, completeStreet );
                                Pubnub.publish({
                                    channel: 'Channel-hkwogxhjp',
                                    message: {
                                        name: 'AUser',
        ...
        function initMapSearch(elat,elong, eaddr) {
            // Init Map
            var mapSearch = new GMaps({
                div: '#js-map-search',
                lat: elat,
            ...
            mapSearch.setCenter(elat, elong);
            mapSearch.addMarker({
                lat: elat,
                lng: elong,
                animation: google.maps.Animation.DROP,
                ...
                infoWindow: {
                    content: '<div class="text-success"><i class="fa fa-map-marker"></i> <strong>'+eaddr+'</strong></div>'
                }
            });
        ...
        // Init Overlay Map
        function initMapOverlay(elat,elong, eaddr){
            new GMaps({
                div: '#js-map-overlay',
                lat: elat,
                lng: elong,
                ...
                lat: elat,
                lng: elong,
                content: '<div class="alert alert-danger alert-dismissable"><h4 class="push-15">You are Here!</h4><p class="push-10">'+eaddr+'</p></div>'
            });
        };
        ...
        // Init Markers Map
        function initMapMarkers(elat,elong, markLat, makrLong, uuid){
            new GMaps({
                div: '#js-map-markers',
                lat: elat,
                lng: elong,
                ...
                {lat: markLat, lng: makrLong, title: uuid, animation: google.maps.Animation.DROP, infoWindow: {content: '<strong>'+uuid+'</strong>'}}
            ]);
        };
        // Init Map with Search functionality
        initMapGeo();
