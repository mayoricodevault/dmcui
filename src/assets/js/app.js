...
App.constant('FBURL', 'https://hcapp.firebaseio.com');
App.constant('_',
    window._
);

App.constant('AwsConfig', {
    AKID: 'AKIAJULQJH5UOSD2CA2Q',
    SAK: 'mqAWQc4mCADla47194TcXHKSRWWr2PAdHdsFU3/0',
    BUCKET: 'redefinemedia',
...
App.config(['$stateProvider', '$urlRouterProvider',
    function ($stateProvider, $urlRouterProvider) {
        ...
        $stateProvider
            ...
            .state('claimsList', {
                url: '/claims/list',
                templateUrl: 'assets/views/claims/list.html',
            ...
            .state('addClaim', {
                url: '/claims/add',
                templateUrl: 'assets/views/claims/edit.html',
                controller: 'ClaimValidationCtrl',
                resolve: {
                    surveysConfig: ["SurveysConfig", function (SurveysConfig) {
                        return SurveysConfig.loadConfig();
                    }],
                    deps: ['$ocLazyLoad', function($ocLazyLoad) {
                        return $ocLazyLoad.load({
                            insertBefore: '#css-bootstrap',
            ...
            .state('formsElements', {
                url: '/forms/elements',
                templateUrl: 'assets/views/forms_elements.html'
            })
            .state('compCharts', {
            ...
            .state('compMapsGooglePN', {
                url: '/components/maps/googlePN',
                templateUrl: 'assets/views/comp_maps_pubnub.html',
                controller: 'CompMapsGooglePNCtrl',
                resolve: {
                    deps: ['$ocLazyLoad', function($ocLazyLoad) {
                        return $ocLazyLoad.load({
                            insertBefore: '#css-bootstrap',
...
App.config(['flowFactoryProvider', function (flowFactoryProvider) {
    ...
    flowFactoryProvider.on('catchAll', function (event) {
        //$rootScope.$emit('catchAll:upload', arguments);
    });
...
App.controller('AppCtrl', ['$scope', '$localStorage', '$window',
    function ($scope, $localStorage, $window) {
        ...
        $scope.appui = {
            ...
            settings: {
                activeColorTheme: 'modern', // Set a color theme of your choice, available: 'app' and 'default'
                ...
                sideOverlayOpen: true, // Visible Side Overlay by default (> 991px)
        ...
        if ($scope.appui.localStorage) {
            // Save/Restore local storage settings
            if ($scope.appui.localStorage) {
                if (angular.isDefined($localStorage.appuiSettings)) {
                    $scope.appui.settings = $localStorage.appuiSettings;
                } else {
                    $localStorage.appuiSettings = $scope.appui.settings;
                }
            }

            // Watch for settings changes
            $scope.$watch('appui.settings', function () {
        ...
        $scope.$watch('appui.settings.activeColorTheme', function () {
            // Handle Color Theme
            $scope.helpers.uiHandleColorTheme($scope.appui.settings.activeColorTheme);
        }, true);

        // Watch for sideScroll variable update
        $scope.$watch('appui.settings.sideScroll', function () {
            // Handle Scrolling
            setTimeout(function () {
